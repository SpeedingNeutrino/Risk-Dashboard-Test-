...
FF5_MAP = {
    ...
    "CMA": "CMA",
    "UMD": "Mom",
}  # RF is not typically mapped here, Mkt-RF is MKT - RF
...
def calc_conditional_var(
    returns: pd.Series, factor_returns: pd.Series, confidence: float = 0.95, window: int = 252
) -> pd.Series:
    """
...
def calc_component_var(returns: pd.DataFrame, weights: pd.Series, confidence: float = 0.95) -> pd.Series:
    """
    ...
    for factor in factor_betas.columns:
        if factor in factor_returns.columns:
            ...
            if len(common_idx) > 60:  # Minimum sample size
                corr = np.corrcoef(lagged_betas.loc[common_idx], factor_returns.loc[common_idx, factor])[0, 1]
                result[factor] = corr
...
def calc_factor_attribution(
    returns: pd.Series, factor_exposures: pd.DataFrame, factor_returns: pd.DataFrame
) -> pd.DataFrame:
    """
    Calculate return attribution by factor
    """
    # Get common dates
    common_dates = returns.index.intersection(factor_exposures.index.intersection(factor_returns.index))
    ...
    for col in factor_contrib.columns:
        factor_contrib[col] = np.where(np.abs(factor_contrib[col]) < 1e-10, 0, factor_contrib[col])

    return factor_contrib


def calc_extreme_scenario(factor_returns: pd.DataFrame, betas: pd.Series, quantile: float = 0.01) -> pd.Series:
    """
...
def fetch_online_factors(start: str) -> Optional[pd.DataFrame]:
    ...
    try:
        with st.sidebar.expander("Fama-French Factor Fetch"):
            ...
            st.success(
                f"Successfully fetched and processed {len(ff)} rows of Fama-French 5 factor data (excluding RF)."
            )
            st.write(ff.tail())
            ff_data = ff
    except Exception as e:
    ...
    try:
        with st.sidebar.expander("Momentum Factor Fetch"):
            ...
            mom.rename(columns={mom.columns[0]: "UMD"}, inplace=True)  # force first col to UMD
        ...
        with st.sidebar.expander("FRED Macro Data Fetch"):
            ...
            if macro_dfs:
                ...
                other_cols_for_pct_change = [
                    col for col in macro.columns if col not in rate_cols_orig_names and not col.endswith("_Diff")
                ]
...
def regress_with_diagnostics(port_r: pd.Series, factors: pd.DataFrame) -> dict:
    ...
    factor_contrib_to_var = pd.Series(
        beta_contrib[0][0], index=["Total Factor"]  # Fixed here - properly extract scalar from numpy array
    )
...
def calc_rolling_betas(port_r: pd.Series, factors: pd.DataFrame, window: int = 126) -> pd.DataFrame:
    ...
    try:
        ...
        for col in rolling_betas.columns:
            ...
            rolling_betas[col] = (
                rolling_betas[col].interpolate(method="linear", limit=5).fillna(method="ffill").fillna(method="bfill")
            )
...
def calc_factor_timing(port_r: pd.Series, factors: pd.DataFrame, window: int = 126) -> pd.DataFrame:
    ...
    for factor in factors.columns:
        if factor in beta_changes.columns:
            ...
            corr, pval = (
                np.corrcoef(beta_changes.loc[common_dates, factor], forward_returns.loc[common_dates, factor])[0, 1],
                0.0,
            )  # Simplification - would calculate actual p-value
...
def run_stress_tests(betas: pd.Series, factors: pd.DataFrame) -> pd.DataFrame:
    ...
    if not relevant_factor_names.empty and not factors.empty and not betas.empty:
        ...
        if (
            not aligned_factors.empty
            and not aligned_betas.empty
            and aligned_factors.shape[0] > 0
            and aligned_factors.shape[1] == aligned_betas.shape[0]
        ):
    ...
    for scenario_name, scenario_data in all_scenarios.items():
        if isinstance(scenario_data, dict):  # Factor shock scenario
            ...
            st.warning(
                f"Unexpected data type for scenario '{scenario_name}': {type(scenario_data)}. Result set to NaN."
            )
...
def show_roll_vol_enhanced(
    portfolio_returns: pd.Series, window: int = 252, secondary_window: int = 63
...
def plot_stress_tests(stress_results: pd.Series) -> Tuple[plt.Figure, plt.Axes]:
    ...
    stress_results.sort_values().plot(
        kind="barh", ax=ax, color=["red" if x < 0 else "green" for x in stress_results.sort_values()]
    )
...
def run_advanced_pca(rets: pd.DataFrame, tickers: list, n_components: int = 5) -> dict:
    ...
    if pca.n_components_ >= 1:
        ...
        var_decomp = pd.DataFrame(
            {"Systematic": systematic_pct, "Idiosyncratic": idiosyncratic_pct}, index=tickers
        )  # Assuming tickers correspond to columns of original rets
...
def dashboard(
    ...
    factor_exposure_smoothing_window: int = 21,
    factor_rolling_return_window: int = 21,
):
    ...
    with col1:
        summ1 = {
            ...
            "Sortino Ratio": (
                (port_r.mean() * 252) / (port_r[port_r < 0].std() * np.sqrt(252))
                if (port_r[port_r < 0].std() > 0)
                else np.nan
            ),
    ...
    for conf in [0.95, 0.99]:
        ...
        ax.axvline(var_value, color="r", linestyle="--", label=f"VaR {conf*100}%: {var_value:.2%}")
    ...
    if factors is not None:
        ...
        if reg_results is not None:
            ...
            fig_contrib, _ = plot_factor_contrib(reg_results["factor_contrib"], reg_results["specific_var"])
    ...
    if pca_results is not None:
        ...
        for pc, data in interp.items():
            ...
            if int(pc[2:]) <= 3:  # First 3 PCs only
                st.write(
                    f"**{pc}:** Positive influence from {', '.join([f'{t[0]} ({t[1]:.2f})' for t in data['positive']])}"
                )
    ...
    factor_data = pd.concat([reg_results["betas"], reg_results["tvalues"], reg_results["pvalues"]], axis=1)
    ...
    st.dataframe(factor_data.style.format({"Beta": "{:.4f}", "t-value": "{:.2f}", "p-value": "{:.4f}"}))
    ...
    with col1:
        ...
        st.dataframe(
            pd.DataFrame(model_summary, index=["Value"]).T.style.format(
                {"Alpha (ann.)": "{:.4%}", "Alpha t-stat": "{:.2f}", "R²": "{:.4f}", "Adj. R²": "{:.4f}"}
            )
        )
    ...
    with col2:
        ...
        st.dataframe(
            pd.DataFrame(risk_metrics, index=["Value"]).T.style.format(
                {
    ...
    if rolling_window >= 30:  # Need at least 30 days
        ...
        if rolling_betas is not None and not rolling_betas.empty:
            ...
            if factor_exposure_smoothing_window > 1:
                smoothed_rolling_betas = rolling_betas.rolling(
                    window=factor_exposure_smoothing_window, min_periods=1
                ).mean()
                st.caption(f"Rolling exposures smoothed with a {factor_exposure_smoothing_window}-day moving average.")

            fig_betas, factors_shown_in_beta_plot = plot_rolling_betas(
                smoothed_rolling_betas, reg_results=reg_results
            )  # MODIFIED LINE
            if fig_betas:
            ...
            if factors_shown_in_beta_plot:  # MODIFIED BLOCK - START
                ...
                if (
                    not significant_factors_to_plot
                    and smoothed_rolling_betas is not None
                    and not smoothed_rolling_betas.empty
                ):
                    if not smoothed_rolling_betas.columns.empty:
                        significant_factors_to_plot = (
                            smoothed_rolling_betas.abs()
                            .mean()
                            .nlargest(min(4, len(smoothed_rolling_betas.columns)))
                            .index.tolist()
                        )
                    else:
            ...
            if significant_factors_to_plot and factors is not None:
                ...
                if available_factors_for_plot:
                    ...
                    aligned_factors_for_plot = factors.loc[
                        port_r.index.intersection(factors.index), available_factors_for_plot
                    ]
                    ...
                    if not aligned_factors_for_plot.empty:
                        ...
                        rolling_compounded_growth = growth_factors.rolling(
                            window=factor_rolling_return_window, min_periods=max(1, factor_rolling_return_window // 2)
                        ...
                        if not rolling_compounded_factor_returns.empty:
                            ...
                            for factor in rolling_compounded_factor_returns.columns:
                                fig_cum_factor_returns.add_trace(
                                    go.Scatter(
                                        x=rolling_compounded_factor_returns.index,
                                        y=rolling_compounded_factor_returns[factor],
                                        mode="lines",
                                        name=factor,
                                    )
                                )
                            ...
                            st.info(
                                f"No data available for rolling {factor_rolling_return_window}-day compounded factor returns plot (possibly due to filtering or data issues)."
                            )
                    else:
                        st.info(
                            "Could not align selected factors (suitable for compounding) with portfolio returns for the rolling compounded returns plot."
                        )
                else:
                    st.info(
                        "None of the selected significant factors are suitable for a compounded return calculation (e.g., they are differences or levels like CPI)."
                    )
    ...
    if stress_results is not None:
        ...
        with st.expander("Detailed Stress Test Results"):
            st.dataframe(
                stress_results.to_frame("Impact")
                .style.format("{:.4%}")
                .background_gradient(cmap="RdYlGn_r", subset=["Impact"])
            )
    ...
    if factors is not None and reg_results is not None:
        ...
        if "rolling_betas" in locals() and rolling_betas is not None and rolling_betas.empty:
            ...
            if not common_idx_attr.empty and not factors_for_attr.loc[common_idx_attr].empty:
                ...
                st.warning(
                    "Could not align rolling betas with portfolio/factor returns for attribution. Falling back if possible."
                )
        ...
        if exposures_for_attribution is None:
            ...
            if static_betas_series is not None and not static_betas_series.empty:
                ...
                if not common_idx_attr.empty and not factors_for_attr.loc[common_idx_attr].empty:
                    ...
                    aligned_static_betas = pd.DataFrame(
                        index=common_idx_attr, columns=static_betas_series.index.intersection(factors_for_attr.columns)
                    )
        ...
        if (
            exposures_for_attribution is not None
            and not exposures_for_attribution.empty
            and "factors_arg_for_attribution" in locals()
        ):
            ...
            factor_attribution = calc_factor_attribution(
                port_r_for_attribution, exposures_for_attribution, factors_arg_for_attribution
            )  # Use aligned factors
            ...
            if factor_attribution is not None and not factor_attribution.empty:
                ...
                attribution_summary = pd.DataFrame(
                    {"Ann. Contribution": avg_contrib, "Percent of Total": pct_contrib * 100}
                )

                st.dataframe(
                    attribution_summary.style.format(
                        {"Ann. Contribution": "{:.4%}", "Percent of Total": "{:.2f}%"}
            ...
            key_metrics.update(
                {
                    ...
                    "Systematic Risk": reg_results["factor_contrib"].sum()
                    / (reg_results["factor_contrib"].sum() + reg_results["specific_var"]),
    ...
    with summary_col2:
        ...
        if reg_results is not None:
            ...
            st.dataframe(
                exposures_df.style.format({"Exposure": "{:.4f}", "t-stat": "{:.2f}"}).background_gradient(
                    cmap="RdYlGn", subset=["Exposure"]
                )
            )
    ...
    if st.button("Generate Report Summary (CSV)"):
        ...
        summary_data = {
            ...
            "Sharpe Ratio": [
                (port_r.mean() * 252) / (port_r.std() * np.sqrt(252)) if port_r.std() > 0 else np.nan,
                "{:.4f}",
            ],
            "Sortino Ratio": [
                (
        ...
        if reg_results is not None:
            factor_data = {
                ...
                "Systematic Risk %": [
                    reg_results["factor_contrib"].sum()
                    / (reg_results["factor_contrib"].sum() + reg_results["specific_var"])
                    * 100,
                    "{:.2f}%",
                ],
...
def main():
    st.set_page_config(page_title="Risk Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
    ...
    start_date = st.sidebar.date_input(
        "Start Date", value=pd.to_datetime(DEFAULT_START), help="Select the start date for analysis"
    )
    ...
    input_method = st.sidebar.radio(
        "Input Method", ["Manual Input", "CSV Upload"], help="Choose how to input your portfolio holdings"
    )
    ...
    if input_method == "Manual Input":
        ...
        weights = pd.Series(
            {
                row["ticker"].strip().upper(): row["weight"]
                for row in updated_rows
                if row["ticker"].strip() and row["weight"] != 0
            }
        )
        ...
        uploaded_file = st.sidebar.file_uploader(
            "Upload Portfolio CSV", type="csv", help="Upload a CSV file with two columns: Ticker and Weight"
        )
    ...
    if run_analysis and weights is not None and not weights.empty:
        ...
        total_row_display = pd.DataFrame(
            {"Weight": [sorted_weights_display.sum()], "Weight (%)": [sorted_weights_display.sum() * 100]},
            index=["TOTAL"],
        )
        ...
        try:
            ...
            if prices_df.empty and all_portfolio_tickers:
                st.error(
                    "Failed to load price data for any tickers in the portfolio. Please check tickers and date range."
                )
                st.stop()
            ...
            if failed_to_load_tickers:
                st.warning(
                    f"Could not load price data for: {', '.join(failed_to_load_tickers)}. These tickers will be removed from the analysis."
                )
        ...
        if not prices_df.empty and weights is not None and not weights.empty:
            dashboard(
                ...
                factors,
                factor_window,
                factor_exposure_smoothing_window,
                factor_rolling_return_window,
            )
        else:
